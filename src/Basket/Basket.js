...
const Basket = ({products, isBasketEmpty, total, dispatch}) => {
      ...
      if ( isBasketEmpty ) {
         ...
         return (
            <Box className = {classes.box}> 
               ...
               {
               products.filter((item) => item.amount > 0 ).map((product, index) => {
                  return (
...
const mapStateToProps = state => {
   return {products: state.products, isBasketEmpty: state.isBasketEmpty, total: state.total}
}
